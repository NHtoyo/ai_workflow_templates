...
- **ドキュメントの即時同期**: 何らかの作業（バグ修正、仕様変更、システム構築、トラブルシューティングなど）が完了した際は、ユーザーから指示されなくても、必ずその結果を関連するドキュメント（`HANDOVER_DOC.md`、`CHANGELOG.md`、各種仕様書など）へ**自発的かつ即座に**反映させること。
- **暗黙知（チャットログ依存）の防止**: エラーの解決策やシステム構成の変更を、AIとユーザーのチャット履歴の中だけで完結させてはならない。AIの会話コンテキストが途切れたり、別エージェントに切り替わったりしてもシステムの状態が正確に把握できるよう、獲得した知識や変更点は必ず「永続的なドキュメントファイル」として物理的に記録すること。
