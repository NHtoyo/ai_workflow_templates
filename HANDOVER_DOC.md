...
## 🔄 HANDOVER_DOC.md と .gemini/rule.md の役割分担

情報の重複や衝突を防ぐため、以下の通り役割を明確に分担して運用します。

* **HANDOVER_DOC.md (人間向け・静的な説明書 / Git管理対象)**
  - **役割**: 人間（ユーザー）がプロジェクトの目的、全体像、依存関係、実行手順、およびAI共同開発フローを把握するためのマニュアル。
  - **管理**: 頻繁には更新せず、システム構成や依存関係に大きな変更があった場合のみ、手動で更新します。
  
* **.gemini/rule.md (AI向け・動的な作業セーブデータ / Git除外対象)**
  - **役割**: AIエージェント自身が、実装した機能の詳細、作成したプログラム一覧、現在までの開発進捗などを記録する履歴ファイル。
  - **管理**: AIが毎セッションの終了時に自動で追記・更新し、次に起動するAIへ作業状態を確実に引き継ぎます。
